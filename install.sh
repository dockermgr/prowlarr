...
# @@File             :  install.sh
# @@Description      :  Container installer script for prowlarr
...
REPO="${DOCKERMGRREPO:-https://github.com/$SCRIPTS_PREFIX}/prowlarr"
APPVERSION="$(__appversion "$REPO/raw/$REPO_BRANCH/version.txt")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Defaults variables
APPNAME="prowlarr"
export INSTDIR="$HOME/.local/share/CasjaysDev/$SCRIPTS_PREFIX/prowlarr"
export DOCKERMGR_CONFIG_DIR="${DOCKERMGR_CONFIG_DIR:-$HOME/.config/myscripts/$SCRIPTS_PREFIX}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the mountpoint directory
export APPDIR="$HOME/.local/share/srv/docker/prowlarr"
...
HUB_IMAGE_URL="casjaysdevdocker/prowlarr"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set container hostname and domain - Default: [prowlarr.$SET_HOST_FULL_NAME] [$SET_HOST_FULL_DOMAIN]
CONTAINER_HOSTNAME=""
...
CONTAINER_ADD_CUSTOM_PORT="0.0.0.0:9696:9696"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
CONTAINER_HOSTNAME="${CONTAINER_HOSTNAME:-${APPNAME:-prowlarr}}"
...
if [ -n "$PROWLARR_USERNAME" ]; then
  CONTAINER_USER_NAME="$PROWLARR_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${PROWLARR_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
...
if [ -n "$PROWLARR_PASSWORD" ]; then
  CONTAINER_USER_PASS="$PROWLARR_PASSWORD"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
  if [ "$CONTAINER_USER_PASS" = "random" ]; then
    CONTAINER_USER_PASS="$(__password "${CONTAINER_PASS_LENGTH:-16}")"
  fi
  CONTAINER_USER_PASS="${PROWLARR_PASSWORD:-${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}}"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
